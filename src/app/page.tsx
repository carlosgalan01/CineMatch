...
function MovieModal({ movie, metadata, rating, onRate, onClose }: { movie: MovieView; metadata?: CatalogMovie; rating?: number; onRate: (value: number) => void; onClose: () => void }) {
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 p-3 backdrop-blur-md sm:grid sm:place-items-center sm:p-6" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><article role="dialog" aria-modal="true" aria-labelledby="movie-title" className="relative mx-auto my-4 grid min-h-[calc(100vh-2rem)] w-full max-w-4xl overflow-hidden rounded-2xl border border-white/13 bg-[#11111d] shadow-[0_30px_100px_rgba(0,0,0,.65)] sm:my-0 sm:min-h-0 sm:grid-cols-[minmax(260px,38%)_1fr]"><div className="relative min-h-[46vh] sm:min-h-[620px]"><Poster movie={movie} metadata={metadata} sizes="(max-width: 640px) 100vw, 340px" priority /><div className="absolute inset-0 bg-gradient-to-t from-[#11111d] via-transparent to-black/15 sm:bg-gradient-to-r sm:from-transparent sm:to-[#11111d]/25" /></div><div className="relative flex flex-col justify-center p-6 sm:p-9 lg:p-12"><button type="button" onClick={onClose} aria-label="Cerrar ficha" className="absolute right-4 top-4 grid h-10 w-10 place-items-center rounded-full border border-white/12 bg-black/25 text-white/65 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b9adff]"><Icon name="close" /></button><p className="eyebrow pr-12">{metadata?.year || "CineMatch"} {metadata?.genres.length ? `· ${metadata.genres.join(" · ")}` : `· ${movie.genre}`}</p><h2 id="movie-title" className="mt-4 text-4xl font-semibold leading-[.95] tracking-[-.05em] sm:text-5xl">{movie.title}</h2>{movie.reason && <div className="mt-5"><span className="reason-badge"><Icon name="spark" className="h-3.5 w-3.5" />{movie.reason}</span></div>}<p className="mt-6 text-sm leading-7 text-white/65 sm:text-base">{metadata?.overview || movie.blurb}</p>{(movie.communityRating || movie.count) && <div className="mt-6 flex gap-6 border-y border-white/8 py-4"><div><p className="text-xl font-semibold text-[#e8c77a]">★ {movie.communityRating}</p><p className="mt-1 text-[10px] uppercase tracking-[.14em] text-white/35">Nota comunidad</p></div><div><p className="text-xl font-semibold">{movie.count}</p><p className="mt-1 text-[10px] uppercase tracking-[.14em] text-white/35">Valoraciones</p></div></div>}<div className="mt-7"><p className="text-xs font-semibold uppercase tracking-[.16em] text-white/45">Tu valoración</p><div className="mt-2"><RatingStars value={rating} onRate={onRate} large /></div><p className="mt-2 text-xs text-white/35">Tu nota se guarda en este dispositivo y reajusta tus recomendaciones.</p></div><div className="mt-7 rounded-xl border border-[#a99bff]/15 bg-[#8f7de8]/[.07] p-4"><div className="flex gap-3"><Icon name="spark" className="mt-0.5 h-4 w-4 shrink-0 text-[#b9adff]" /><div><p className="text-xs font-semibold text-[#d5cdff]">Por qué podría gustarte</p><p className="mt-1 text-xs leading-5 text-white/45">{movie.reason || "Forma parte de tu selección inicial y ayudará al motor a entender mejor tus afinidades."}</p></div></div></div></div></article></div>;
}
